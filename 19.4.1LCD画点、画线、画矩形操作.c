...
static void lcd_draw_line(unsigned int x, unsigned int y, int dir, unsigned int length, unsigned int color)
{
    unsigned short rgb565_color = argb8888_to_rgb565(color);//得到 RGB565 颜色值
    unsigned int end;
    unsigned long temp;     // 从(0,0)开始，横着数，直到起始点(x,y)的像素点个数
    ...
    { //水平线
        ...
        if (end >= width)       // 对结束点进行制约
            end = width - 1;
        for ( ; x <= end; x++, temp++)  // 开始划线
            screen_base[temp] = rgb565_color;
    }
    else
    { //垂直线
        end = y + length - 1;
        if (end >= height)      // 对结束点进行制约
            end = height - 1;
        for ( ; y <= end; y++, temp += width)   // 开始划线
            screen_base[temp] = rgb565_color;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    /* 获取参数信息 */
    ioctl(fd, FBIOGET_VSCREENINFO, &fb_var);    // 获取 FrameBuffer 设备的可变参数信息
    ioctl(fd, FBIOGET_FSCREENINFO, &fb_fix);    // 获取 FrameBuffer 设备的固定参数信息
    screen_size = fb_fix.line_length * fb_var.yres; // 一行的字节数 * 行数 = 整个屏幕的字节数
    width = fb_var.xres;    // 可视区域，一行有多少个像素点，X 分辨率
    height = fb_var.yres;   // 可视区域，一列有多少个像素点，Y 分辨率
