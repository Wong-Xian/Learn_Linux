...
<b>虽然进程结束会自动释放内存，但还是建议调用free函数手动释放，养成良好习惯。</b>

### 7.6.2 在堆上分配内存的其他方法：calloc函数

calloc()函数用来动态地分配内存空间并初始化为 0。函数原型：
...
实质：虚拟文件系统。为应用层访问系统内核数据提供了接口，用户和应用程序可以通过 proc 文件系统得到系统信息和进程相关信息。

文件本身并不存在于磁盘当中、只存在于内存当中,与 devfs 一样，都被称为虚拟文件系统。
