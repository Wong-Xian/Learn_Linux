...
以下函数可以设置当前进程的信号掩码（要阻塞信号的集合）。是<b>系统调用</b>
...
与之对应的概念是标准信号，即阻塞状态下如果一个信号产生了多次，在阻塞信号集中也只记录一次，最终传递给进程的也只有一次。

实时信号：
- 信号范围扩大，支持自定义信号。
- 队列化管理，阻塞时收到多次信号，会多次传递给进程。
- 当发送一个实时信号时，可为信号指定伴随数据（一整形数据或者指针值），供接收信号的进程在它的信号处理函数中获取。
- 保障信号传递顺序。按照信号编号从小到大传递。信号值越小，优先级越高。
